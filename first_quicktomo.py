...
roi_col_start = 197
roi_col_end = 1882
rad_stack = rad_stack_0[0,roi_row_start:roi_row_end,:,roi_col_start:roi_col_end]
...
plt.figure()
plt.scatter(np.linspace(1,img_x_bounds[1]-img_x_bounds[0],img_y_bounds[1]-img_y_bounds[0]),sinavg)
...
pixel_dist = 1
tomo_3d = np.zeros([stack.shape[1],reconstruction_fbp.shape[0],reconstruction_fbp.shape[1]])
#%%
...
#def tomo_reconstruct_layer(rad_stack,cross_sectional_dim,layer_row=1024,start_tomo_ang=0., end_tomo_ang=360.,tomo_num_imgs=360, center=0.,pixel_size=0.00148):
    ...
    max_rad=int(cross_sectional_dim/effective_pixel/2.*1.0) #10% slack to avoid edge effects
...
#%%
reconstruction_matrix = np.zeros([10,reconstruction_fbp.shape[0],reconstruction_fbp.shape[1]])
...
plt.imshow(filters.gaussian(reconstruction_matrix[layer_no,:,:],0.2),vmin = 0.0005, vmax = 0.002)   
#######################################
#%%
##########SLOW TOMO HERE to BOTTOM#############

sinograms = np.zeros([1,rad_stack.shape[0],rad_stack.shape[1],rad_stack.shape[2]])
