...
def _guessVBounds(image, area = 1):
    ...
    image = image[len(image)//4:len(image)*3//4,len(image[0])//4:len(image[0])*3//4]
...
def reconstruct(sinograms, centers, imageBounds, layers, theta, sigma = .1, ncore = 4, algorithm = 'gridrec',run_secondary_sirt=False,secondary_iter=100):
    ...
    if np.isscalar(centers):
        ...
        for x in range(layers[1]-layers[0]):
            arr[x] += centers[0] + (centers[1]-centers[0])/float(layers[1]-layers[0])*x
        centers = arr
    else:
    ...
    if len(sinograms.shape) == 4:
        ...
        recons = np.zeros([len(sinograms), layers[1]-layers[0], imageBounds[1]-imageBounds[0],imageBounds[1]-imageBounds[0]])
        recon_clean=np.zeros([len(sinograms), layers[1]-layers[0], imageBounds[1]-imageBounds[0],imageBounds[1]-imageBounds[0]])
        for el in range(len(sinograms)):
            print("Process Element #" + str(el))
	    for x in range(layers[1]-layers[0]):            
                #if x %1 == 0:
                print("Processing Layer #" + str(x))
                tmp=tomopy.prep.stripe.remove_stripe_fw(sinograms[el,:,layers[0]+x:layers[0]+x+1,imageBounds[0]:imageBounds[1]],sigma=sigma,ncore=ncore)
                ...
                if run_secondary_sirt:
                    ...
                    recon = tomopy.recon(tmp, theta, center = (imageBounds[1] - imageBounds[0])/2.0 + centers[x],init_recon=tmp_recon,algorithm=tomopy.astra,options=options,sinogram_order=False,ncore=ncore)
                    recons[el][x] += recon[0]
                else:
        ...
        recons = np.zeros([1,layers[1]-layers[0], imageBounds[1]-imageBounds[0],imageBounds[1]-imageBounds[0]])
        recon_clean = np.zeros([1,layers[1]-layers[0], imageBounds[1]-imageBounds[0],imageBounds[1]-imageBounds[0]])
        for x in range(layers[1]-layers[0]):
            #if x %1 == 0:
            print("Process Layer #" + str(x))
            tmp=tomopy.prep.stripe.remove_stripe_fw(sinograms[:,layers[0]+x:layers[0]+x+1,imageBounds[0]:imageBounds[1]],sigma=sigma,ncore=ncore)
            tmp_recon = tomopy.recon(tmp, theta, center = (imageBounds[1] - imageBounds[0])/2.0 + centers[x], algorithm=algorithm,sinogram_order=False,ncore=ncore)
...
#        self.fig1 = fig1
        ...
        self.element = 0
        self.recon = reconstruct(sinograms[self.element], self.center, imageBounds, self.layer + self.layerBounds[0], theta, sigma, ncore, algorithm)
...
#        self.fig1.cla() #to keep in shape
        self.fig2.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = self.vMin, vmax = self.vMax, cmap = self.cmap)
#        self.fig1.set_title("Previous Version")
        ...
        #define sliders
        self.minSlide = Slider(axMin, 'vMin', self.vMin, self.vMin + (self.vMax-self.vMin)/2, valinit= self.vMin)
        self.maxSlide = Slider(axMax, 'vMax', self.vMin + (self.vMax-self.vMin)/2, self.vMax, valinit= self.vMax)
        self.centerSlide = Slider(axCenter, 'center',max(-200,(imageBounds[0]-imageBounds[1])/2), min(200,(imageBounds[1]-imageBounds[0])/2), valinit=0.00)
        ...
        #define textboxs
        self.layerButton = TextBox(axLayer, "layer ", color = axcolor, initial = str(self.layer))
        self.layerButton.on_submit(self.layerSubmit)
        self.elementButton = TextBox(axElement, "element ", color = axcolor, initial = "0")
        self.elementButton.on_submit(self.elementSubmit)
        self.centerButton = TextBox(axCenterButton, "center ", color = axcolor, initial = "0")
        self.centerButton.on_submit(self.centerSubmit)
        
        #define undo
        axUndo = plt.axes([.85,.05,.09,.06])
        self.undoButton = Button(axUndo, 'Undo', color = axcolor, hovercolor = '0.975')
        self.undoButton.on_clicked(self.undo)
        self.versions = [[self.center, self.imageBounds, self.layer, self.vMin, self.vMax, self.element]]
...
#        self.fig1.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = self.vMin, vmax = self.vMax, cmap = self.cmap, interpolation = self.interpolation)
        self.recon = reconstruct(self.sinograms[self.element], self.center, self.imageBounds, self.layer + self.layerBounds[0], self.theta, self.sigma, self.ncore, self.algorithm)
        self.fig2.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = self.vMin, vmax = self.vMax, cmap = self.cmap, interpolation = self.interpolation)        
        self.versions += [[self.center, self.imageBounds, self.layer, self.vMin, self.vMax, self.element]]
...
#        self.fig1.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = self.vMin, vmax = self.vMax, cmap = self.cmap, interpolation = self.interpolation)
        ...
        self.fig2.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = self.minSlide.val, vmax = self.maxSlide.val, cmap = self.cmap, interpolation = self.interpolation)
        self.minSlide.valtext.set_text(_toSci(self.minSlide.val))
        self.maxSlide.valtext.set_text(_toSci(self.maxSlide.val))
    ...
    def elementSubmit(self, text):
        ...
        try:
            ...
            if(el in range(0,len(self.sinograms))):
                self.element = el
#                self.fig1.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = self.minSlide.val, vmax = self.maxSlide.val, cmap = self.cmap, interpolation = self.interpolation)
                self.recon = reconstruct(self.sinograms[self.element], self.center, self.imageBounds, self.layer + self.layerBounds, self.theta, self.sigma, self.ncore, self.algorithm)
                self.fig2.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = self.vMin, vmax = self.vMax, cmap = self.cmap, interpolation = self.interpolation)
                self.versions += [[self.center, self.imageBounds, self.layer, self.vMin, self.vMax, self.element]]
    ...
    def undo(self, val):
        ...
        if(len(self.versions) >= 3):
            ...
            tempRecon = reconstruct(self.sinograms[self.element], newVer[0],newVer[1],newVer[2],self.theta, self.sigma, self.ncore, self.algorithm)
            self.fig1.imshow(img.filters.gaussian_filter(tempRecon[0][0],0.75), vmin = newVer[3], vmax = newVer[4], cmap = self.cmap, interpolation = self.interpolation)
        else:
#            self.fig1.cla()
            self.fig1.set_title("Previous Version")
        if(len(self.versions)>=2):
            oldVer = self.versions[-2]
            self.recon = reconstruct(self.sinograms[self.element], oldVer[0],oldVer[1],oldVer[2],self.theta, self.sigma, self.ncore, self.algorithm)
            self.fig2.imshow(img.filters.gaussian_filter(self.recon[0][0],0.75), vmin = oldVer[3], vmax = oldVer[4], cmap = self.cmap, interpolation = self.interpolation)
...
class BoundHelper:
    ...
    def __init__(self, radiograph, vMin, vMax, cmap, interpolation):
        ...
        plt.imshow(self.radiograph,vmin=self.vMin,vmax=self.vMax, cmap = self.cmap, interpolation = self.interpolation)
        ...
        #define texts
        self.prompt = plt.text(len(self.radiograph[0]) // 2,-len(self.radiograph)//15,"Pick Top Left Coordinate.", fontsize = 15, horizontalalignment = 'center')
        self.display = plt.text(len(self.radiograph[0]) //2,len(self.radiograph)*1.1,"Coordinate 1: []      Coordinate 2: []", fontsize = 10, horizontalalignment = 'center')
        ...
        if(x is not None and y is not None):
            if((self.x-x)**2 + (self.y-y)**2 < 50):    
                ...
                if(len(self.temp) <= 2):
                    ...
                    self.display.set_text("Coordinate 1: " + str(self.temp[0]) + "     " + " Coordinate 2: []")
                    self.ax.plot(x,y, 'ro')
                    
                if(len(self.temp) == 2):
                    self.layerBounds = [self.temp[0][1],self.temp[1][1]]
                    self.imageBounds = [self.temp[0][0],self.temp[1][0]]
                    rect = patches.Rectangle((self.imageBounds[0],self.layerBounds[0]),self.imageBounds[1]-self.imageBounds[0],self.layerBounds[1]-self.layerBounds[0],edgecolor = "red", facecolor = 'none',linewidth = 1)
                    self.ax.add_patch(rect)
                    self.prompt.set_text("Selected " + str(self.layerBounds[1]-self.layerBounds[0]) + " layers, and " + str(self.imageBounds[1]-self.imageBounds[0]) + " columns.")
                    self.display.set_text("Coordinate 1: " + str(self.temp[0])  + "     " + " Coordinate 2: " + str(self.temp[1]))
...
#    return arr
    ...
    arr=np.linspace(topCenter,bottomCenter,num=layerBounds[1]-layerBounds[0])
    return arr
...
class Giffer:
    def __init__(self, volume, cmap):
        ...
        self.im = plt.imshow(volume[self.index], vmin = bounds[0], vmax = bounds[1], cmap = cmap, animated = True)
        self.ani = animation.FuncAnimation(self.fig,self.updatefig,interval = 100, blit=True)
        self.ax = plt.gca()
...
def genDark(tdf_data_folder, tdf_fold_start = None,num2skip = None,
    ...
    if(tdf_fold_start is not None and num2skip is not None and tdf_num_imgs is not None):
        ...
        for x in range(len(tdf_img_nums)):
            tdf_stack[x,:,:]=img.imread(tdf_data_folder+'nf_%0.6d.tif' %(tdf_img_nums[x]))
    ...
    #finding the meta data
        folder = os.listdir(tdf_data_folder[:len(tdf_data_folder)-3] + 'scalars')
        metaData = np.loadtxt(tdf_data_folder[:len(tdf_data_folder)-3] + 'scalars/' + folder[0] + '/summary.dat')
        theta = metaData[:,1]
...
#                print(tdf_data_folder+'nf_%0.6d.tif' %(int(firstImage) + x))
                tdf_stack[x,:,:]=img.imread(tdf_data_folder+'nf_%0.6d.tif' %(int(firstImage) + x ))
            else:
                counter += 1
...
def genBright(tbf_data_folder, tdf, tbf_fold_start = None, num2skip = None, tbf_num_imgs = None):
    if(tbf_fold_start is not None and num2skip is not None and tbf_num_imgs is not None):
        tbf_img_nums=np.arange(tbf_fold_start+num2skip, tbf_fold_start+num2skip+tbf_num_imgs,1)
        ...
        for ii in np.arange(tbf_num):
            tbf_stack[ii,:,:]=img.imread(tbf_data_folder+'nf_%0.6d.tif' %(tbf_img_nums[ii]))-tdf
            #image_stack[ii,:,:]=np.flipud(tmp_img>threshold)
        tbf=np.median(tbf_stack,axis=0)
        print('complete!')
        return tbf
    else:
        folder = os.listdir(tbf_data_folder[:len(tbf_data_folder)-3] + 'scalars')
        metaData = np.loadtxt(tbf_data_folder[:len(tbf_data_folder)-3] + 'scalars/' + folder[0] + '/summary.dat')
        theta = metaData[:,1]
        ...
        for x in range(0, len(theta)-1):
            if(theta[x] > 0):
                tbf_stack[x,:,:]=img.imread(tbf_data_folder+'nf_%0.6d.tif' %(int(firstImage) + x + 2)) - tdf
                #image_stack[x,:,:]=np.flipud(tmp_img>threshold)
            else:
                counter += 1
...
def genTomo(tomoDataFolder, tdf, tbf, img_x_bounds, img_y_bounds, intCorr = None, tomo_fold_start = None,num2skip = None, tomo_num_imgs = None, theta = None):
    ...
    if(tomoDataFolder is None):    
        ...
        metaData = np.loadtxt(tomoDataFolder[:len(tomoDataFolder)-3] + 'scalars/' + folder[0] + '/summary.dat')
    if(theta is None):
        theta = metaData[:,1]
    if(num2skip is None):
        num2skip = 2
    ...
    tomo_img_nums=np.arange(tomo_fold_start+num2skip,tomo_fold_start+num2skip+tomo_num_imgs,1)
    tomoImgs=np.zeros([1,tomo_num_imgs,(img_x_bounds[1]-img_x_bounds[0]),(img_y_bounds[1]-img_y_bounds[0])])
    ...
    for ii in np.arange(num_tomoImgs):
        ...
        #loads data
        tmp_img=img.imread(tomoDataFolder+'nf_%0.6d.tif' %(tomo_img_nums[ii]))
        #removes negative and applies intensity corrections
        tmp_img0=tmp_img[img_x_bounds[0]:img_x_bounds[1],img_y_bounds[0]:img_y_bounds[1]]
        tmp_img2=tomopy.misc.corr.remove_neg(tmp_img0-tdf[img_x_bounds[0]:img_x_bounds[1],img_y_bounds[0]:img_y_bounds[1]], val=0.0,)*intCorr[ii]

        #normalizes in some way idk
        tomoImgs[0,ii,:,:]=tomopy.prep.normalize.minus_log(tmp_img2/tbf[img_x_bounds[0]:img_x_bounds[1],img_y_bounds[0]:img_y_bounds[1]])
    print("complete!")
